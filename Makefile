...
DEV_AGENT_ENV := set PG_DSN=$(DEV_PG_DSN)&& set AGENT_GRPC_ADDR=127.0.0.1:$(DEV_AGENT_GRPC_PORT)&& set REDIS_ADDR=127.0.0.1:$(DEV_REDIS_PORT)&& set DEEPSEEK_API_KEY=$(DEEPSEEK_API_KEY)&& set DEEPSEEK_MODEL=$(DEEPSEEK_MODEL)&&
DEV_GATEWAY_ENV := set AGENT_GRPC_ADDR=127.0.0.1:$(DEV_AGENT_GRPC_PORT)&& set GATEWAY_HTTP_ADDR=:$(DEV_GATEWAY_HTTP_PORT)&& set REDIS_ADDR=127.0.0.1:$(DEV_REDIS_PORT)&& set WECHAT_TOKEN=$(WECHAT_TOKEN)&& set WECHAT_APPID=$(WECHAT_APPID)&& set WECHAT_APPSECRET=$(WECHAT_APPSECRET)&&
PRO_AGENT_ENV := set PG_DSN=$(PRO_PG_DSN)&& set AGENT_GRPC_ADDR=127.0.0.1:$(PRO_AGENT_GRPC_PORT)&& set REDIS_ADDR=127.0.0.1:$(PRO_REDIS_PORT)&& set DEEPSEEK_API_KEY=$(DEEPSEEK_API_KEY)&& set DEEPSEEK_MODEL=$(DEEPSEEK_MODEL)&&
...
DEV_AGENT_ENV := PG_DSN="$(DEV_PG_DSN)" AGENT_GRPC_ADDR="127.0.0.1:$(DEV_AGENT_GRPC_PORT)" REDIS_ADDR="127.0.0.1:$(DEV_REDIS_PORT)" DEEPSEEK_API_KEY="$(DEEPSEEK_API_KEY)" DEEPSEEK_MODEL="$(DEEPSEEK_MODEL)"
DEV_GATEWAY_ENV := AGENT_GRPC_ADDR="127.0.0.1:$(DEV_AGENT_GRPC_PORT)" GATEWAY_HTTP_ADDR=":$(DEV_GATEWAY_HTTP_PORT)" REDIS_ADDR="127.0.0.1:$(DEV_REDIS_PORT)" WECHAT_TOKEN="$(WECHAT_TOKEN)" WECHAT_APPID="$(WECHAT_APPID)" WECHAT_APPSECRET="$(WECHAT_APPSECRET)"
PRO_AGENT_ENV := PG_DSN="$(PRO_PG_DSN)" AGENT_GRPC_ADDR="127.0.0.1:$(PRO_AGENT_GRPC_PORT)" REDIS_ADDR="127.0.0.1:$(PRO_REDIS_PORT)" DEEPSEEK_API_KEY="$(DEEPSEEK_API_KEY)" DEEPSEEK_MODEL="$(DEEPSEEK_MODEL)"
...
check-secrets: env
ifeq ($(OS),Windows_NT)
	@if "$(DEEPSEEK_API_KEY)"=="" (echo DEEPSEEK_API_KEY must be set in .env & exit /b 1)
...
db: env
	$(COMPOSE_DEV) pull postgres redis migrate
...
run-agent-dev: check-secrets
	$(DEV_AGENT_ENV) ./bin/agent.exe

run-gateway-dev: check-secrets
	$(DEV_GATEWAY_ENV) ./bin/gateway.exe

dev: db build
	@echo "Dev database is ready."
	@echo "Run these in separate terminals:"
	@echo "  make run-agent-dev"
	@echo "  make run-gateway-dev"

run-agent-pro: check-secrets
	$(PRO_AGENT_ENV) ./bin/agent.exe

run-gateway-pro: check-secrets
	$(PRO_GATEWAY_ENV) ./bin/gateway.exe
