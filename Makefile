...
TEST_ENV := set PG_DSN=$(DEV_PG_DSN)&& set REDIS_ADDR=127.0.0.1:$(DEV_REDIS_PORT)&&
...
DEV_GATEWAY_ENV := set AGENT_GRPC_ADDR=127.0.0.1:$(DEV_AGENT_GRPC_PORT)&& set GATEWAY_HTTP_ADDR=:$(DEV_GATEWAY_HTTP_PORT)&& set REDIS_ADDR=127.0.0.1:$(DEV_REDIS_PORT)&& set PG_DSN=$(DEV_PG_DSN)&& set AUTH_PEPPER=$(AUTH_PEPPER)&& set SMTP_HOST=$(SMTP_HOST)&& set SMTP_PORT=$(SMTP_PORT)&& set SMTP_IMPLICIT_TLS=$(SMTP_IMPLICIT_TLS)&& set SMTP_USERNAME=$(SMTP_USERNAME)&& set SMTP_PASSWORD=$(SMTP_PASSWORD)&& set SMTP_FROM=$(SMTP_FROM)&& set COOKIE_SECURE=$(COOKIE_SECURE)&& set WECHAT_ENABLED=$(WECHAT_ENABLED)&& set WECHAT_TOKEN=$(WECHAT_TOKEN)&& set WECHAT_APPID=$(WECHAT_APPID)&& set WECHAT_APPSECRET=$(WECHAT_APPSECRET)&&
...
PRO_GATEWAY_ENV := set AGENT_GRPC_ADDR=127.0.0.1:$(PRO_AGENT_GRPC_PORT)&& set GATEWAY_HTTP_ADDR=:$(PRO_GATEWAY_HTTP_PORT)&& set REDIS_ADDR=127.0.0.1:$(PRO_REDIS_PORT)&& set WECHAT_ENABLED=$(WECHAT_ENABLED)&& set WECHAT_TOKEN=$(WECHAT_TOKEN)&& set WECHAT_APPID=$(WECHAT_APPID)&& set WECHAT_APPSECRET=$(WECHAT_APPSECRET)&&
else
...
TEST_ENV := PG_DSN="$(DEV_PG_DSN)" REDIS_ADDR="127.0.0.1:$(DEV_REDIS_PORT)"
...
DEV_GATEWAY_ENV := AGENT_GRPC_ADDR="127.0.0.1:$(DEV_AGENT_GRPC_PORT)" GATEWAY_HTTP_ADDR=":$(DEV_GATEWAY_HTTP_PORT)" REDIS_ADDR="127.0.0.1:$(DEV_REDIS_PORT)" PG_DSN="$(DEV_PG_DSN)" AUTH_PEPPER="$(AUTH_PEPPER)" SMTP_HOST="$(SMTP_HOST)" SMTP_PORT="$(SMTP_PORT)" SMTP_IMPLICIT_TLS="$(SMTP_IMPLICIT_TLS)" SMTP_USERNAME="$(SMTP_USERNAME)" SMTP_PASSWORD="$(SMTP_PASSWORD)" SMTP_FROM="$(SMTP_FROM)" COOKIE_SECURE="$(COOKIE_SECURE)" WECHAT_ENABLED="$(WECHAT_ENABLED)" WECHAT_TOKEN="$(WECHAT_TOKEN)" WECHAT_APPID="$(WECHAT_APPID)" WECHAT_APPSECRET="$(WECHAT_APPSECRET)"
...
PRO_GATEWAY_ENV := AGENT_GRPC_ADDR="127.0.0.1:$(PRO_AGENT_GRPC_PORT)" GATEWAY_HTTP_ADDR=":$(PRO_GATEWAY_HTTP_PORT)" REDIS_ADDR="127.0.0.1:$(PRO_REDIS_PORT)" WECHAT_ENABLED="$(WECHAT_ENABLED)" WECHAT_TOKEN="$(WECHAT_TOKEN)" WECHAT_APPID="$(WECHAT_APPID)" WECHAT_APPSECRET="$(WECHAT_APPSECRET)"
endif

.PHONY: env db dev pro docker test migrate-up migrate-down logs ps clean build run-agent-dev run-gateway-dev run-agent-pro run-gateway-pro check-secrets k3s-manifests
